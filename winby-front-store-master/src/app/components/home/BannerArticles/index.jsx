...
import { Button, BoxCard } from '../Styled'
...
export const BannerArticles = ({ data, handleClick, itemType, noSeeMore }) => <ShadowCard>
    <Slider>
        {data?.length ? data.map(x => (
            <Link
                key={ x.uscs_id || x.uscp_id || x.usp_id || x.uss_id }
                to={{ state: { item: x }, pathname: `/${ x.uscs_id || x.uss_id ? 'servicios' : 'productos' }/${ x.usp_id || x.uss_id ? 'detalles' : 'categorias' }/${ x.uscs_id || x.uscp_id || x?.product?.p_id || x?.service?.s_id }` }}
    ...
    {!noSeeMore && <Button onClick={handleClick}>Ver Más <IconArrowRight size='10px' color={PColor} style={{ marginLeft: 5 }} /></Button>}
</ShadowCard>