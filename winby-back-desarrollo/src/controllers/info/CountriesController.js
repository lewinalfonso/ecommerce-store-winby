...
controllers.getOne = async (req, res) => {
    ...
    const data = await CountriesModel.findOne({ attributes: ['c_name', 'c_calCod', 'c_state'], where: { c_id: deCode(c_id), c_state: c_state !== 'false' ? c_state : { [Op.gte]: 0 } } }).catch((x) => error = true)

    /** respuesta */
    if (error) { return res.json({ success: 0, error }) } else if (data) { return res.json({ success: 1, data }) } else { return res.json({ success: 0, error, message: 'No se ha encontrado ningún resultado.' }) }
}
...
controllers.getAll = async (req, res) => {
    ...
    /** verificando que parametro es para hacer la consulta */
    const data = await CountriesModel.findAll({ attributes: ['c_id', 'c_name', 'c_calCod', 'c_state'], where: { c_state: c_state !== 'false' ? c_state : { [Op.gte]: 0 } }, order: [['c_name', 'ASC']] }).catch((x) => error = true)
...
controllers.register = async (req, res) => {
    ...
    const { c_name, c_calCod } = req.body; let error = false

    /** Guardar el registro */
    const data = await CountriesModel.create({ c_name, c_calCod, c_state: 1 }).catch((x) => error = true)
...
controllers.edit = async (req, res) => {
    /** creacion de variables, y recibiendo los datos */
    const { c_id, c_name, c_calCod } = req.body; let error = false

    /** Editar el registro */
    await CountriesModel.update({ c_name, c_calCod }, { where: { c_id: deCode(c_id) } }).catch((x) => error = true)
...
controllers.changeState = async (req, res) => {
    ...
    const data = await CountriesModel.findOne({ attributes: ['c_state'], where: { c_id: deCode(c_id) } }).catch((x) => error = true)

    /** Verifica si existe para actualizar */
    if (data !== null && data !== true) { await CountriesModel.update({ c_state: data.dataValues.c_state ? 0 : 1 }, { where: { c_id: deCode(c_id) } }).catch((x) => error = true) }
